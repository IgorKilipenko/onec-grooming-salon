...
Функция ПолучитьЦенуНоменклатуры(Знач номенклатураСсылка, Знач поставщикСсылка, Знач моментВремени = Неопределено) Экспорт
    ...
    ДиагностикаКлиентСервер.Утверждение(результат = Неопределено ИЛИ результат.Количество() = 1,
            СтрШаблон(
                "Результатом выполнения запроса функции ""ПолучитьЦенуНоменклатуры"" должна быть только одна запись цены номенклатуры.
                |Фактическое количество полученных записей ""%1"".", результат.Количество()));
...
Функция КонвертироватьТаблицуЦенНоменклатурыВСоответствие(Знач таблицаЦенНоменклатуры) Экспорт
