...
// Устарела. Функцию нужно переделать
//
// Параметры:
...
Функция получитьДанныеРегистра(Знач видЗапроса, Знач структураПолей, Знач параметры, выгрузить = Неопределено)
    ...
    Если видЗапроса = "История" Тогда
        запрос = сформироватьЗапросОбороты(структураПолей, параметры);
    ...
    ИначеЕсли видЗапроса = "СрезПоследнихИнтервал" Тогда
        запрос = сформироватьЗапросСрезПоследнихИнтервал(параметры);
    Иначе
...
Функция сформироватьЗапросСрезПоследних(Знач структураПолей, Знач параметры)
    ...
    график = ?(параметры.Свойство("ГрафикРаботы"), параметры.ГрафикРаботы, Неопределено);
    ...
    Если график <> Неопределено Тогда
        условие = СтрШаблон("%1 %2%3", условие, ?(условие <> Неопределено, "И ", ""), "ГрафикРаботы = &ГрафикРаботы");
...
// Устарела. Функцию нужно переделать используя срезы первых и последних
//
// Параметры:
//  структураПолей - Структура, ФиксированнаяСтруктура, Неопределено
//  параметры - Структура, ФиксированнаяСтруктура
...
// Возвращаемое значение:
//  - Запрос
Функция сформироватьЗапросОбороты(Знач структураПолей, Знач параметры)
...
// Устарела. Необходимо выполнить оптимизацию запроса - ограничить возможный размер выборки
//
// Параметры:
//  структураПолей - Структура, ФиксированнаяСтруктура, Неопределено
//  параметры - Структура, ФиксированнаяСтруктура
...
// Возвращаемое значение:
//  - Запрос
Функция сформироватьЗапросСрезПоследнихИнтервал(Знач параметры)
    #Область ДиагностикаАргументов
    ДиагностикаКлиентСервер.Утверждение(
        ТипЗнч(параметры) = Тип("Структура") ИЛИ ТипЗнч(параметры) = Тип("ФиксированнаяСтруктура"),
            "Аргумент ""Параметры"" должен иметь тип ""[Фиксированная]Структура"".");
    ...
    //
    //! Необходимо выполнить рефакторинг. Ограничит размер выборки например отбором по группе сотрудников.
    //! Нужно добавить условие для СрезПоследних_НачалоПериода.Работает = Истина и далее ограничить
    //! СрезПоследних_КонецПериода этой выборкой.
    //! Порционную выбору делать не хочется т.к. это не соответствует механикам отбора данных других методов из программного интерфейса
    //
    ...
    запрос.Текст =
        ...
        |	КадроваяИсторияСотрудниковСрезПоследних.ГрафикРаботы,
        |	КадроваяИсторияСотрудниковСрезПоследних.Оклад,
        |	КадроваяИсторияСотрудниковСрезПоследних.Работает
        |ИЗ
        ...
        |////////////////////////////////////////////////////////////////////////////////
        |ВЫБРАТЬ
        |	ВТ_КадроваяИстория.Период КАК Период,
        |	ВТ_КадроваяИстория.Сотрудник КАК Сотрудник,
        |	ВТ_КадроваяИстория.ГрафикРаботы КАК ГрафикРаботы,
        |	ВТ_КадроваяИстория.Оклад КАК Оклад,
        |	ВТ_КадроваяИстория.Работает КАК Работает
        |ИЗ
        |	ВТ_КадроваяИстория КАК ВТ_КадроваяИстория
        |
        |СГРУППИРОВАТЬ ПО
        |	ВТ_КадроваяИстория.Сотрудник,
        |	ВТ_КадроваяИстория.Период,
        |	ВТ_КадроваяИстория.ГрафикРаботы,
        |	ВТ_КадроваяИстория.Оклад,
        |	ВТ_КадроваяИстория.Работает
        |
        |УПОРЯДОЧИТЬ ПО
