...
// Возвращаемое значение:
//  - Булево
Функция ПолучитьУслугаОказана(Знач записьКлиентаСсылка) Экспорт
    Возврат ЗначениеЗаполнено(Документы.ЗаписьКлиента.ПолучитьДокументРеализацииНаОсновании(записьКлиентаСсылка));
КонецФункции
...
Функция РассчитатьДлительностьОказанияУслуг(Знач списокНоменклатуры, Знач минимальноеВремяОказанияУслугиВМинутах = Неопределено) Экспорт
    ...
    Если ТипЗнч(списокНоменклатуры) = Тип("Массив") Тогда
        услугиТЗ = РаботаСКоллекциями.КонвертироватьМассивВТаблицуЗначений(списокНоменклатуры, "Услуга",
