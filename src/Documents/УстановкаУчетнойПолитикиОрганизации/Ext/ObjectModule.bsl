...
Процедура ОбработкаПроведения(_, __)
    исходнаяУчетнаяПолитика = получитьТекущуюУчетнуюПолитику();
	Движения.УчетнаяПолитика.Записывать = Истина;
	выполнитьДвижениеПоУчетнаяПолитика();

	Если УчетнаяПолитика = Перечисления.ВидыУчетнойПолитики.ПоСредней
        И исходнаяУчетнаяПолитика = Перечисления.ВидыУчетнойПолитики.FEFO Тогда
...
Процедура выполнитьДвижениеПоУчетнаяПолитика()
	движение = ЭтотОбъект.Движения.УчетнаяПолитика.Добавить();
...
Процедура выполнитьДвижениеПоСкладам(строкаОстатки)
	движение = создатьДвижениеПоСкладам(ЭтотОбъект.Дата, ВидДвиженияНакопления.Расход);
	ЗаполнитьЗначенияСвойств(движение, строкаОстатки);

	движение = создатьДвижениеПоСкладам(ЭтотОбъект.Дата, ВидДвиженияНакопления.Приход);
...
Функция получитьТекущуюУчетнуюПолитику()
