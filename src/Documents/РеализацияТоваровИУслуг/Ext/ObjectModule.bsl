...
Функция получитьВыборкуНоменклатураДокументаИОстатки(Знач менеджерТаблиц)
    ...
    Возврат Документы.РеализацияТоваровИУслуг.ПолучитьВыборкуТоваровДокументаИОстатки(
...
Функция выполнитьВсеДвиженияДокумента()
    ...
    Если получитьЭтоРеализацияНаОснованииЗаписиКлиента() И  Тогда
        выполнитьДвижениеЗаказыКлиентовРасход(выборкаПартияНоменклатуры);
        ...
        Если результатКонтроляОстатковПоЗаказу.Отказ Тогда
            сообщитьПользователюОПревышенииОстатков(результатКонтроляОстатковПоЗаказу);
            результатВыполнения.Отказ = Истина;
        КонецЕсли;
    КонецЕсли;
...
// Возвращаемое значение:
//	- ФиксированнаяСтруктура - Структура вида { Отказ: Булево, Остаток: Число | Неопределено}
Функция выполнитьКонтрольОстатковДляЗаказыКлиентов()
    ...
    остатокПоЗаказу = Документы.РеализацияТоваровИУслуг.ПолучитьОстатокСуммыПоЗаказуКлиента(
            ЭтотОбъект.ДокументОснование, Новый Граница(ЭтотОбъект.МоментВремени()));

    ДиагностикаКлиентСервер.Утверждение(остатокПоЗаказу <> Неопределено,
        "Ошибка! Не удалось получить остатки по записи клинта.");
    Если остатокПоЗаказу = Неопределено Тогда
        ВызватьИсключение СтрШаблон("Не удалось получить остатки для записи клинта ""%1"".", Строка(ЭтотОбъект.ДокументОснование));
    КонецЕсли;

    остатокСуммы = ?(остатокПоЗаказу = Неопределено, Неопределено, остатокПоЗаказу.Сумма);
    Возврат Новый ФиксированнаяСтруктура("Отказ, Остаток", остатокСуммы <> Неопределено ИЛИ остатокСуммы < 0, остатокСуммы);
КонецФункции
#КонецОбласти // Движения
...
Процедура сообщитьПользователюОПревышенииОстатков(
    ...
    Если записьКлиентаПредставление <> Неопределено Тогда // Это превышение суммы по записи клиента
        текстСообщения = СтрШаблон("Превышена сумма по записи клиента ""%1"" на ""%2""руб.",
                Формат(превышениеОстатков, форматФинансовыхДанных), записьКлиентаПредставление);
    Иначе
