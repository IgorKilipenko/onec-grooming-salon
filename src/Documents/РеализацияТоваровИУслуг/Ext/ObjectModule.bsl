...
Процедура выполнитьВсеДвиженияДокумента(отказ = Ложь)
    запретПроведенияПриОтрицательныхОстатках_ = Константы.ЗапретПроведенияПриОтрицательныхОстатках;
    ...
    Пока выборкаНоменклатураДокумента_.Следующий() Цикл
        ...
        Если выборкаНоменклатураДокумента_.ЭтоТовар Тогда
            ...
            стоимостьОбщая_ = 0;
            несписанныйОстаток_ = выборкаНоменклатураДокумента_.КоличествоВДокументе;
            выборкаНоменклатураДокументаПоСрокуГодности_ = выборкаНоменклатураДокумента_.Выбрать();
            Пока выборкаНоменклатураДокументаПоСрокуГодности_.Следующий() И несписанныйОстаток_ > 0 Цикл
                стоимостьОбщая_ = стоимостьОбщая_ + выполнитьДвижениеТоварыНаСкладахРасход(
...
// Устарела.
// [прим. разработчика] Нужно выполнить рефакторинг: избавиться от использования выходного параметра {результатОстатокВДокументе}
//
// Параметры:
//  выборкаНоменклатураДокументаДетальные - Выборка
//  результатОстатокВДокументе - Число - !!! Используется как Выходной параметр
//
// Возвращаемое значение:
//  - Число - Сумма выполненного списания. В аргументе возвращает величину несписанного остатка
//
Функция выполнитьДвижениеТоварыНаСкладахРасход(выборкаНоменклатураДокументаДетальные, результатОстатокВДокументе) // => Число
    доступноДляСписания_ = Мин(выборкаНоменклатураДокументаДетальные.КоличествоОстаток, результатОстатокВДокументе);

    движение_ = Движения.ТоварыНаСкладах.ДобавитьРасход();
    движение_.Период = Дата;
    движение_.Номенклатура = выборкаНоменклатураДокументаДетальные.Номенклатура;
    движение_.Склад = выборкаНоменклатураДокументаДетальные.Склад;
    движение_.СрокГодности = выборкаНоменклатураДокументаДетальные.СрокГодности;
    движение_.Количество = доступноДляСписания_;
    Если доступноДляСписания_ = выборкаНоменклатураДокументаДетальные.КоличествоОстаток Тогда
        движение_.Сумма = выборкаНоменклатураДокументаДетальные.СуммаОстаток;
    Иначе
        движение_.Сумма = доступноДляСписания_ / выборкаНоменклатураДокументаДетальные.КоличествоОстаток
            * выборкаНоменклатураДокументаДетальные.СуммаОстаток;
    КонецЕсли;

    результатОстатокВДокументе = результатОстатокВДокументе - доступноДляСписания_;

    Возврат движение_.Сумма;
КонецФункции
