...
Процедура выполнитьВсеДвиженияДокумента(отказ = Ложь)
    ...
    Пока выборкаНоменклатураДокумента_.Следующий() Цикл
        ...
        Если выборкаНоменклатураДокумента_.ЭтоТовар Тогда
            ...
            текущийОстатокВДокументе_ = выборкаНоменклатураДокумента_.КоличествоВДокументе;
            выборкаНоменклатураДокументаПоСрокуГодности_ = выборкаНоменклатураДокумента_.Выбрать();
            стоимостьОбщая_ = 0;

            количествоГруппТоваров_ = выборкаНоменклатураДокументаПоСрокуГодности_.Количество();
            Пока выборкаНоменклатураДокументаПоСрокуГодности_.Следующий() И текущийОстатокВДокументе_ > 0 Цикл
                результатСписания_ = выполнитьДвижениеТоварыНаСкладахРасход(
                        выборкаНоменклатураДокументаПоСрокуГодности_,
                        текущийОстатокВДокументе_);

                стоимостьОбщая_ = стоимостьОбщая_ + результатСписания_.Сумма;
                текущийОстатокВДокументе_ = текущийОстатокВДокументе_ - результатСписания_.Количество;

                количествоГруппТоваров_ = количествоГруппТоваров_ - 1;
                Если количествоОстатков_ < 0 И количествоГруппТоваров_ = 0 И (НЕ запретПроведенияПриОтрицательныхОстатках_) Тогда
                    выполнитьДвижениеТоварыНаСкладахРасход(
                        выборкаНоменклатураДокументаПоСрокуГодности_,
                            текущийОстатокВДокументе_, Истина);
                КонецЕсли;
            КонецЦикла;
