...
Функция очиститьФиоПоМаске(Знач стрФио)
    результатПоиска = РаботаСоСтрокамиВызовСервера.НайтиПоРегулярномуВыражению(стрФио, "[а-яА-ЯЁё\-]+");
    Если результатПоиска.НачальнаяПозиция = 0 Тогда
        Возврат "";
    КонецЕсли;

    стрФио = РаботаСоСтрокамиВызовСервера.ЗаменитьПоРегулярномуВыражению(результатПоиска.Значение, "\s+", "");
    результатПоиска = РаботаСоСтрокамиВызовСервера.НайтиПоРегулярномуВыражению(
