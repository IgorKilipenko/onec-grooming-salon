...
Функция очиститьФиоПоМаске(Знач стрФио)
    результатПоиска = СтрНайтиПоРегулярномуВыражению(стрФио, "[а-яА-ЯЁё\-]+");
    Если результатПоиска.НачальнаяПозиция = 0 Тогда
        Возврат "";
    КонецЕсли;

    стрФио = СтрЗаменитьПоРегулярномуВыражению(результатПоиска.Значение, "\s+", "");
    результатПоиска = СтрНайтиПоРегулярномуВыражению(стрФио, "^(?:[^а-яА-ЯЁё]+)?([а-яА-ЯЁё]+)((?:\-[а-яА-ЯЁё]+)?)(?:[^а-яА-ЯЁё]+)?$");
