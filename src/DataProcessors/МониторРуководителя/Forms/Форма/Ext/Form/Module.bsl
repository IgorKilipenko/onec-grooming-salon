...
Процедура ПриОткрытии(_)
    ...
    заполнитьДанные();
    ОбновитьЛентуНовостей(Неопределено);
КонецПроцедуры
...
Процедура ОбновитьЛентуНовостей(_)
    // Проверка заполнения API KEY сервиса новостей
    Если получитьКлючСервисаНовостейДоступенНаСервере() = Истина Тогда
        ЭтотОбъект.Новости = получитьНовостиНаСервере(
                ЭтотОбъект.НовостиСтрокаПоиска,
                УниверсальноеВремя(ЭтотОбъект.НовостиДатаЗагрузки),
                ЭтотОбъект.НовостиЯзыкПоиска,
                ЭтотОбъект.НовостиКоличество);

    Иначе // Сервис новостей недоступен
        сообщение = Новый СообщениеПользователю;
        сообщение.Текст = "Для загрузки новостной ленты необходимо заполнить API ключ сервиса новостей ""NEWSAPI"".
            |Ключ можно установить на вкладке ""Настройки"".";
        сообщение.Сообщить();
    КонецЕсли;
...
&НаСервереБезКонтекста
Функция получитьКлючСервисаНовостейДоступенНаСервере()
    Возврат ЗначениеЗаполнено(Константы.API_KEY_newsapi.Получить());
КонецФункции
