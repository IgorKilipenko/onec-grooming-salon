...
Функция ЗагрузитьНовости(Знач текстЗапроса, Знач датаЗагрузки = Неопределено, Знач язык = "ru", Знач максКоличество = 100) Экспорт
    ...
    Если датаЗагрузки <> Неопределено Тогда
        параметрыЗапроса = параметрыЗапроса + СтрШаблон("&from=%1T%2",
                Формат(датаЗагрузки, "ДФ=yyyy-MM-dd"),
                Формат(датаЗагрузки, "ДФ=hh:mm:ss"));
    КонецЕсли;
...
Функция ЗагрузитьКурсыВалютПоДаннымЦентробанка(Знач датаЗагрузки) Экспорт
    сервер = "cbr.ru";
    ...
    строкаXML = результатЗапроса.Ответ.ПолучитьТелоКакСтроку();
    результат = ПрочитатьИзXMLПоследовательно(строкаXML);
...
Функция ПрочитатьИзXMLПоследовательно(Знач строкаXML)
    чтениеXML = Новый ЧтениеXML;
    чтениеXML.УстановитьСтроку(строкаXML);

    результат = Новый Структура("USD, EURO, KZT");
    найденоВалют = 0;
    текущаяВалюта = Неопределено;
    Пока чтениеXML.Прочитать() И найденоВалют < результат.Количество() Цикл
        ...
        Если текущаяВалюта = Неопределено ИЛИ результат[текущаяВалюта] <> Неопределено Тогда
            Продолжить;
        КонецЕсли;

        этоИскомаяВалюта = чтениеXML.ТипУзла = ТипУзлаXML.НачалоЭлемента И чтениеXML.Имя = "VunitRate";
        Если этоИскомаяВалюта Тогда
            чтениеXML.Прочитать();
            результат[текущаяВалюта] = ПреобразованиеТиповКлиентСервер.ПривестиСтрокуКЧислу(чтениеXML.Значение);
            найденоВалют = найденоВалют + 1;
        КонецЕсли;
